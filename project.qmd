...
for url in recap_urls:
    ...
    for idx in range(1, len(tables), 2):
        ...
        if raw.shape[1] not in (expected_small, expected_large):
            print(f"Skipping table — unexpected column count: {raw.shape[1]}")
...
```{python}
# Save full score dataset
combined_scores.to_csv("WGI_2025_scores.csv", index=False)

# Save full rank dataset
combined_ranks.to_csv("WGI_2025_ranks.csv", index=False)
```